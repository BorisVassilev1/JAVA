...
public class Matrices {//много математика за ротация на вектори

	public static Vector3f rotateVec3f(Vector3f vec, Vector3f rot) {
	...
	public static Vector3f rotateVec2f(Vector3f vec, float radians)
	{
		float[][] rotationZ = { { (float) Math.cos(radians), (float) -Math.sin(radians), 0 },
				{ (float) Math.sin(radians), (float) Math.cos(radians), 0 }, { 0, 0, 1 } };
		vec = matrixToVec(matMul(rotationZ, vec));
		return vec;
	}
	...
	public static Vector3f degToRad(Vector3f deg) {
		...
		result.scale((float) (Math.PI / 180));
		return result;
	}
